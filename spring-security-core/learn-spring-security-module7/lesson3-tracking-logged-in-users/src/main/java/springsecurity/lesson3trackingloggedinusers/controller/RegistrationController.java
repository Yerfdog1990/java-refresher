package springsecurity.lesson3trackingloggedinusers.controller;
...
import springsecurity.lesson3trackingloggedinusers.persistance.model.*;
import springsecurity.lesson3trackingloggedinusers.persistance.repository.SecurityQuestionDefinitionRepository;
import springsecurity.lesson3trackingloggedinusers.persistance.repository.SecurityQuestionRepository;
import springsecurity.lesson3trackingloggedinusers.persistance.service.ActiveUserService;
import springsecurity.lesson3trackingloggedinusers.persistance.service.IStudentService;
import springsecurity.lesson3trackingloggedinusers.registration.OnRegistrationCompleteEvent;
...
public class RegistrationController {
    ...
    @Autowired
    public RegistrationController(IStudentService studentService, ApplicationEventPublisher eventPublisher, JavaMailSender mailSender, Environment env, SecurityQuestionDefinitionRepository securityQuestionDefinitionRepository, SecurityQuestionRepository securityQuestionRepository, ActiveUserService activeUserService) {
    ...
    public ModelAndView showUsers() {
        ...
        List<StudentDTO> studentDTOList = allActiveUsers.stream().map(StudentDTO::new).collect(Collectors.toList());
        return new ModelAndView("users", "students", studentDTOList);
    }
    ...
    public ModelAndView showUser(@PathVariable("id") Long id) {
        List<String> allActiveUsers = activeUserService.getAllActiveUsers();
        List<StudentDTO> studentDTOList = allActiveUsers.stream().map(StudentDTO::new).collect(Collectors.toList());
        return new ModelAndView("view", "student", studentDTOList);
    }

    @GetMapping("/users/edit")
    public String handleDoubleSlashEdit() {
