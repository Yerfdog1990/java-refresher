package springsecurity.lesson5jdbcauthentication.controller;
...
import springsecurity.lesson5jdbcauthentication.exceptions.DuplicateStudentException;
import springsecurity.lesson5jdbcauthentication.persistence.Mapper.StudentMapper;
import springsecurity.lesson5jdbcauthentication.persistence.dto.StudentRequestDto;
import springsecurity.lesson5jdbcauthentication.persistence.dto.StudentResponseDto;
import springsecurity.lesson5jdbcauthentication.persistence.entity.Student;
import springsecurity.lesson5jdbcauthentication.persistence.service.StudentService;
    ...
    public String update(@PathVariable Long id, @Valid @ModelAttribute("student") StudentRequestDto studentDto, BindingResult result, RedirectAttributes ra) {
        ...
        if (toUpdate.getPassword() == null || toUpdate.getPassword().isBlank()) {
            ...
            if (existing != null) {
                ...
                // Preserve current authorities/role if not provided in the update form
                if (studentDto.getRole() == null || studentDto.getRole().isBlank()) {
                    toUpdate.setAuthorities(existing.getAuthorities());
                }
            }
        }
