...
⚠️ If an authentication provider does not use `UserDetailsService` (for example, LDAP), Remember-Me will **not work** unless a `UserDetailsService` bean is present.
...
If a token is captured, it can be reused from any user agent until it expires—similar to digest authentication.

Mitigation:

* Changing the user’s password immediately invalidates all issued tokens
...
```
 ...
 └────────┬─────────┘
          │
          │ 6. Request WITHOUT JSESSIONID
          │    BUT WITH remember-me cookie
          ▼
 ┌────────────────────────────┐
...
```
 ...
 └────────┬─────────┘
          │
          │ 6. Later request WITHOUT session
...
* **series** → Identifies the login session
* **token** → Rotates on every successful auto-login
...
## 2️⃣ PersistentTokenRepository Bean
...
## 3️⃣ RememberMeServices Bean (CORE)
...
## 4️⃣ RememberMeAuthenticationProvider Bean
...
⚠️ **The key must match** the one used in `RememberMeServices`.

---

## 5️⃣ Security Filter Chain Configuration
...
## 6️⃣ UserDetailsService (MANDATORY)
...
## 7️⃣ Login Form (Frontend MUST match)
...
## 8️⃣ What Happens at Runtime (Mapped to Flow)
...
✔ HTTPS-only cookie support

---

## 🔐 Cookie-Based vs Persistent (Quick Reminder)
...
```
┌──────────────────────────────────────────────┐    ┌──────────────────────────────────────────────┐
│        COOKIE-BASED REMEMBER-ME              │    │        PERSISTENT TOKEN REMEMBER-ME          │
│        (Token in Cookie Only)                │    │        (Token Stored in Database)            │
└──────────────────────────────────────────────┘    └──────────────────────────────────────────────┘


1️⃣ LOGIN WITH "REMEMBER ME" CHECKED
┌──────────────┐                              ┌──────────────┐
│   Browser    │                              │   Browser    │
└──────┬───────┘                              └──────┬───────┘
...
└──────────┬───────────────┘              └──────────┬───────────────────────┘
           ...
           ▼                                         ▼
┌──────────────────────────┐              ┌──────────────────────────────────┐
│ TokenBasedRememberMe     │              │ PersistentTokenBasedRememberMe   │
│ Services                 │              │ Services                         │
└──────────┬───────────────┘              └──────────┬───────────────────────┘
           │                                         │
           │ Create hash-based token                 │ Create (series + token)
           │                                         │
           ▼                                         ▼
┌──────────────────────────┐              ┌──────────────────────────────────┐
│ Set remember-me cookie   │              │ Save token in DATABASE           │
│ (username + expiry +     │              │ (series, token, last_used)       │
│ signature)               │              └──────────┬───────────────────────┘
└──────────┬───────────────┘                         │
           │                                         ▼
           │                              ┌──────────────────────────────────┐
...
2️⃣ NEXT REQUEST (NO SESSION COOKIE)

┌──────────────┐                              ┌──────────────┐
│   Browser    │                              │   Browser    │
└──────┬───────┘                              └──────┬───────┘
       │ Request with remember-me cookie             │ Request with remember-me cookie
       ▼                                             ▼
┌──────────────────────────┐              ┌──────────────────────────────────┐
│ RememberMeAuthentication │              │ RememberMeAuthentication         │
│ Filter                   │              │ Filter                           │
└──────────┬───────────────┘              └──────────┬───────────────────────┘
           │                                         │
           │ Validate token hash                     │ Lookup series in DB
           │                                         │
           │                                         ▼
           │                              ┌──────────────────────────────────┐
           │                              │ PersistentTokenRepository        │
...
└──────────┬───────────────┘
           ...
           ▼
┌──────────────────────────┐              ┌──────────────────────────────────┐
│ New JSESSIONID issued    │              │ New JSESSIONID issued            │
│ User remains logged in   │              │ User remains logged in           │
└──────────────────────────┘              └──────────────────────────────────┘
...
❌ Cookie-Based → demos, low-risk apps  
✅ Persistent Token → production systems
```
