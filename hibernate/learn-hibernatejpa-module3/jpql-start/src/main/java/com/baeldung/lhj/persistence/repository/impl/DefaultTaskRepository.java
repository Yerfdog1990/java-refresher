    ...
    public List<Task> findByStatuses(List<TaskStatus> statuses) {
        try (EntityManager entityManager = JpaUtil.getEntityManager()) {
            return entityManager
                    .createQuery("SELECT t FROM Task t WHERE t.status IN (:statuses)", Task.class) // -> named parameters – prefixed with a colon followed by a name, like :status or :campaignId.
                   //.createQuery("SELECT t FROM Task t WHERE t.status IN (?1)", Task.class) // -> positional parameters – specified using a question mark followed by a number that indicates the parameter’s position (like ?1, ?2)
