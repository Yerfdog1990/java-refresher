...
public class BaseClass {
    ...
    public void setup(String os, String browser) throws IOException {
        ...
        if ("remote".equals(normalizedEnv)) {
            ...
            switch (normalizedBrowser) {
                ...
                case "edge" -> capabilities.setBrowserName("microsoft-edge");
            ...
            switch (normalizedBrowser) {
                ...
                case "safari" -> {
                    ...
                    // Safari-specific: Add extra time for driver initialization
                    try {
                        Thread.sleep(2000);
        ...
        // Safari-specific: Use longer implicit wait for stability
        if ("safari".equals(normalizedBrowser)) {
        ...
        // Safari-specific: Add delay before maximize and validate window
        if ("safari".equals(normalizedBrowser)) {
            try {
                Thread.sleep(3000);
            } catch (InterruptedException e) {
    ...
    protected boolean isSafariBrowser() {
        return getDriver() instanceof SafariDriver;
    }

    protected void validateSafariWindow() {
        if (isSafariBrowser()) {
            try {
                // Safari-specific: Validate window is still active
