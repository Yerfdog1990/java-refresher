...
public class AccountModelAssembler
    ...
    public EntityModel<Account> toModel(Account entity) {
        return EntityModel.of(entity,
            linkTo(methodOn(AccountApi.class).getOne(entity.getId())).withSelfRel(),
            linkTo(methodOn(AccountApi.class).listAll()).withRel(IanaLinkRelations.COLLECTION),
            linkTo(methodOn(AccountApi.class).deposit(entity.getId(), null)).withRel("deposits"),
            linkTo(methodOn(AccountApi.class).withdraw(entity.getId(), null)).withRel("withdrawal")
        );
    }
}
```

This centralizes all HATEOAS logic and eliminates duplication.

---

## 15. Adding Arbitrary Links

```java
entityModel.add(Link.of("https://company.xyz", "ref"));
collectionModel.add(Link.of("http://company.com/api/docs", "docs"));
collectionModel.add(Link.of("http://company.com/name", IanaLinkRelations.AUTHOR));
```
...
* Add **exam questions**
* Compare **Level 2 vs Level 3 controllers**
